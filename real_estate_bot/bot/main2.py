...
ADMIN_CHANNEL_ID = os.getenv('ADMIN_CHANNEL_ID', '@your_admin_channel')
...
if ADMIN_CHANNEL_ID == '@your_admin_channel':
    logger.warning("⚠️ Please set ADMIN_CHANNEL_ID in .env file for admin approval!")
    exit(1)
...
def migrate_database():
    ...
    try:
        ...
        if 'admin_channel_message_id' not in columns:
            cursor.execute('ALTER TABLE listings ADD COLUMN admin_channel_message_id INTEGER')
            logger.info("Added admin_channel_message_id column")
...
def format_listing_for_admin_channel(listing) -> str:
    """Format listing for admin channel approval"""
...
def get_admin_channel_keyboard(listing_id: int) -> InlineKeyboardMarkup:
    """Create approval keyboard for admin channel"""
    builder = InlineKeyboardBuilder()
    builder.add(InlineKeyboardButton(text="✅ Tasdiqlash", callback_data=f"channel_approve_{listing_id}"))
    builder.add(InlineKeyboardButton(text="❌ Rad etish", callback_data=f"channel_decline_{listing_id}"))
    builder.adjust(2)
    return builder.as_markup()

async def send_to_admin_channel(listing_id: int):
    ...
    keyboard = get_admin_channel_keyboard(listing_id)
    ...
    try:
        ...
        # Save admin channel message ID
        conn = sqlite3.connect('real_estate.db')
        cursor = conn.cursor()
        cursor.execute(
            'UPDATE listings SET admin_channel_message_id = ? WHERE id = ?',
            (message.message_id, listing_id)
        )
        conn.commit()
        conn.close()
...
# =============================================
# ADMIN CHANNEL APPROVAL HANDLERS - NEW
# =============================================

@dp.callback_query(F.data.startswith('channel_approve_'))
async def admin_channel_approve(callback_query):
    ...
    try:
        await callback_query.message.edit_text(
            f"✅ E'lon #{listing_id} tasdiqlandi!\n\n"
            f"👤 Admin: {callback_query.from_user.first_name}\n"
    ...
    await callback_query.answer("✅ E'lon tasdiqlandi va kanalga yuborildi!")

@dp.callback_query(F.data.startswith('channel_decline_'))
async def admin_channel_decline(callback_query, state: FSMContext):
    ...
    # Store listing ID and admin ID for feedback
    await state.set_state(AdminStates.writing_feedback)
    await state.update_data(
        listing_id=listing_id,
        admin_channel_message_id=callback_query.message.message_id
    )
    
    # Ask for feedback in PM
    try:
        await bot.send_message(
            chat_id=callback_query.from_user.id,
            text=f"❌ E'lon #{listing_id} rad etish sababi:\n\n"
                 f"Foydalanuvchiga yuborilishi uchun sabab yozing:"
        )
        await callback_query.answer("📝 Sabab yozish uchun botga o'ting")
    except:
        await callback_query.answer("❌ Botga yoza olmaysiz! Avval /start bosing")

@dp.message(AdminStates.writing_feedback)
async def process_admin_channel_feedback(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    
    data = await state.get_data()
    listing_id = data.get('listing_id')
    admin_channel_message_id = data.get('admin_channel_message_id')
    feedback = message.text
    
    # Update listing status
    update_listing_approval(listing_id, 'declined', message.from_user.id, feedback)
    
    # Get listing for user notification
    listing = get_listing_by_id(listing_id)
    if listing:
    ...
    try:
        await bot.edit_message_text(
            chat_id=ADMIN_CHANNEL_ID,
            message_id=admin_channel_message_id,
            text=f"❌ E'lon #{listing_id} rad etildi!\n\n"
...
async def admin_stats_handler(message: Message):
    ...
    try:
        ...
        stats_text = f"""📊 Admin Panel Statistika:

⏳ Kutilayotgan e'lonlar: {pending_count} ta
📅 Bugungi e'lonlar: {today_count} ta  
👥 Jami foydalanuvchilar: {users_count} ta

🔧 Admin kanal: {ADMIN_CHANNEL_ID}
📢 Asosiy kanal: {CHANNEL_ID}"""
...
async def finish_listing(callback_query, state: FSMContext):
    ...
    await send_to_admin_channel(listing_id)
...
async def main():
    ...
    if ADMIN_CHANNEL_ID != '@your_admin_channel':
        logger.info(f"✅ Admin channel configured: {ADMIN_CHANNEL_ID}")
    else:
        logger.warning("⚠️ Admin channel not configured! Set ADMIN_CHANNEL_ID in .env")
